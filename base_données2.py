...
lieu=c.fetchall()   # r est une liste de listes, chacune de ses sous listes contenant le nom de la station, sa latitude et sa longitude
...
valeur_forte_liste=[]
valeur_faible_liste=[]
valeur_tres_faible_liste=[]
dixieme_module_liste=[]
debit_donnee_provisoire_m3_liste=[]
c.execute('SELECT code_hydro FROM hydro_debit_histo')
station=c.fetchall()

for i in station:
   
    c.execute('SELECT valeur_tres_faible,valeur_faible,valeur_forte,dixieme_module,debit_donnee_provisoire_m3 FROM hydro_debit_histo WHERE code_hydro=? ',(i))
    r=c.fetchall()
  
    valeur_forte_liste.append(r[2])  # toutes les valeurs fortes par station, ordonnées par date
    valeur_tres_faible_liste.append(r[0])
    valeur_faible_liste.append(r[1])
    dixieme_module_liste.append(r[3])
    debit_donnee_provisoire_m3_liste.append(r[4])
  
#y=([a[2] for a in r])
#plt.plot(y)
#plt.title('valeur_forte')
